...
Editor.prototype.open = Promise.method(function (path, again) {
  var self = this;
  path = util.resolvePath(path);
  return self.ready // ensures textBuf path-changed is triggered in _initHandlers
    .then(function () { try{return self.textBuf.setPath(path)}catch(e){logger.warn('here', JSON.stringify(e))} })
    .then(function () { return self.textBuf.load(); })
    .then(function () { self.selection.setHeadPosition(new Point(0, 0)); })
    .catch(function (err) {
      if (err.message === "Unable to watch path" && !again) return self.open(path, true); // try again
      if (!err.code) err.code = (err.message.match(/^[A-Z]+\b/) || [])[0];
      logger.warn('here', err.message, err.code, err.message === "Unable to watch path" && !again);
      switch (err.code) {
        case 'EACCES':
    ...
    .catch(function (here) { logger.warn('yoyoyoyo', here); });
});
Editor.prototype.save = Promise.method(function (path, again) {
  var self = this;
  var header = self.slap.header;
  return Promise.resolve()
    .then(function () {
      return path
        ? self.textBuf.saveAs(util.resolvePath(path))
        : self.textBuf.save();
    })
    .then(function () {
    ...
    .catch(function (err) {
      if (err.message === "Unable to watch path" && !again) return self.save(path, true); // try again
      switch (err.code) {
